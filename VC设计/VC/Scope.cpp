...
void CScope::DrawAxis(CDC *pDC)
{
	...
	if(nPosY<=m_rectCurve.top+10)
		nPosY += 10;
	if(0.01<=m_dDimY&&m_dDimY<0.1)
	pDC->TextOut(m_rectCurve.left-2,nPosY,CString("0.00"));
	if(0.1<=m_dDimY&&m_dDimY<1)
	...
	for(i=1;i<=4;i++)
	{
		...
		if(nPosY<=m_rectCurve.bottom)
		{
			if(0.001<=m_dDimY&&m_dDimY<0.01)
				str.Format("%.3f",-i*m_dDimY);
			else if(0.01<=m_dDimY&&m_dDimY<0.1)
				str.Format("%.2f",-i*m_dDimY);
			else if(0.1<=m_dDimY&&m_dDimY<1)
...
void CScope::DrawCurve1(CDC *pDC)
{
	m_bDrawing = TRUE;
	int nSize = m_TimeList.size();
...
//	pDC->Polyline(pPointArray,nSize);
	...
	for(i=0;i<nSize;i++)
	{
		pDC->MoveTo(pPointArray[i].x,nCenterY);
		pDC->LineTo(pPointArray[i].x,pPointArray[i].y);
	}
	pDC->SelectClipRgn(NULL);
	pDC->SelectObject(pOldPen);
